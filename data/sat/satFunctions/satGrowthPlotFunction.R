## PSAT 2019 Growth Reading ----
## PSAT 2019 Growth Math ----
SATGrowthPlotFunction <- function() {
  p <-  ggplot() +
    geom_bar(data= CMASHundredMGP, aes(x = group, fill = percent),
             position="fill",
             width = 0.15) +
    geom_point_interactive(data = SATvar, mapping = aes(y = PctPts, 
                                                        tooltip = paste0('<b>Rating: </b>', RATING_FINAL)), 
                           x = 1,  
                           size = 15, 
                           alpha = 0.5,
                           shape = 17)+
    geom_text(aes(y = SATvar$PctPts, label = ordinal(SATvar$GRO_MEDIAN_SGP)), 
              x = 1, 
              vjust = -2, 
              size = 15)+
...
noSATGrowthHere <- function(){
  p <-  ggplot() +
    geom_text(aes(y = 0.35, 
                  x = 1.2, 
                  label = 'PSAT not tested here'),
              size = 15)+
    scale_fill_manual(values = c('#3688c8', '#22a783', '#e2a331', '#d8274a'))+
    coord_flip()+
    ylim(0, 1)+
